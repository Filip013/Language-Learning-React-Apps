...
class SweepTab extends StatefulWidget {
  const SweepTab({super.key});

  @override
  State<SweepTab> createState() => _SweepTabState();
}

class _SweepTabState extends State<SweepTab> {
  int _currentIdx = 0;
  ...
  // ---------------------------------------------------------------------------
  // Data & auto-navigation (mirrors React SweepTab)
  // ---------------------------------------------------------------------------

  List<dynamic> _sweepItems(CourseProvider prov) {
    final sweep = prov.activeEpisode?['sweep'];
    if (sweep is List) return sweep;
    return const [];
  }
  ...
  void _maybeAutoNavigate(CourseProvider prov) {
    ...
    final items = _sweepItems(prov);
    if (items.isNotEmpty) {
      final rev = prov.sweepRevealed;
      var firstUnfinished = -1;
      for (var i = 0; i < items.length; i++) {
        if (rev['sweep_$i'] != true) {
          firstUnfinished = i;
          break;
        }
      }
  ...
  String get _qId => 'sweep_$_currentIdx';

  // ---------------------------------------------------------------------------
  // Keyboard shortcuts (mirrors React SweepTab key map)
  // ---------------------------------------------------------------------------
  ...
  bool _handleGlobalKey(KeyEvent event) {
    ...
    if (courseProv.activeTab != 'sweep') return false;
    if (_sweepItems(courseProv).isEmpty) return false;
  ...
  void _handleNext(CourseProvider prov) {
    final total = _sweepItems(prov).length;
  ...
  /// Speak [word. target, english, target] and on completion mark the item as
  /// revealed + mastered (React behavior).
  void _playSweep(CourseProvider prov, String qId) {
    final item = _itemAt(_sweepItems(prov), _currentIdx);
    final word = item['word']?.toString() ?? '';
    final target = item[prov.config.primaryTextKey]?.toString() ?? '';
    final english = item['english']?.toString() ?? '';
    if (target.isEmpty) return;

    final texts = <String>['$word. $target', english, target];
    prov.speakListTexts(texts, qId).then((_) {
      prov.updateFirebase({
        'sweepMastered': {...prov.sweepMastered, qId: true},
        'sweepRevealed': {...prov.sweepRevealed, qId: true},
      });
    });
  }
  ...
  void _resetSweep(CourseProvider prov) {
    setState(() {
      _showConfirmReset = false;
      _currentIdx = 0;
    });
    _scrollToTop();
    prov.updateFirebase({'sweepMastered': {}, 'sweepRevealed': {}});
  }
  ...
  Widget build(BuildContext context) {
    ...
    final items = _sweepItems(courseProv);
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.monitor_heart_outlined, size: 48, color: textSecondary.withValues(alpha: 0.4)),
    ...
    final qId = _qId;
    final isRevealed = courseProv.sweepRevealed[qId] == true;
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: Stack(
                        ...
                        children: [
                          const TabBadge(icon: Icons.monitor_heart_outlined, label: 'DIAGNOSTIC SWEEP'),
                          Positioned(
                            right: 0,
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Row(
                          children: List.generate(items.length, (idx) {
                            ...
                            final isCompleted = courseProv.sweepRevealed['sweep_$idx'] == true;
                              ...
                              Row(
                                children: [
                                  ...
                                  PlayButton(
                                    ...
                                    onPressed: () => _playSweep(courseProv, qId),
                                  ),
                                ],
                              ),
                            ],
              ...
              Positioned.fill(
                child: Center(
                  child: Material(
                    ...
                    child: InkWell(
                      onTap: () => _playSweep(courseProv, qId),
