        ...
        ws.onopen = () => {
            const setupMessage = {
                setup: {
                    ...
                    systemInstruction: { parts: [{ text: "You are a bilingual text-to-speech reader. Read the text provided aloud exactly as written. Switch naturally between European Portuguese and English based on the text. Do not translate. Do not add any conversational filler or introductions. Provide clear pauses between sentences." }] }
                }
            };
...
export default function Portuguese() {
  ...
  useEffect(() => {
    if (!user) return;
    const unsubLex = db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('database').doc('lexicon')
      .onSnapshot(snap => {
        setGlobalDictionary(snap.exists && snap.data().entries ? snap.data().entries : []);
      });

    const unsubEps = db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('episodes')
  ...
  useEffect(() => {
    ...
    db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('episodes').doc(activeEpisodeId)
      .get().then(snap => { if (snap.exists) setActiveEpisode(snap.data()); });
    
    const unsubProg = db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('progress').doc(activeEpisodeId)
      .onSnapshot(snap => {
        if (snap.exists) {
          const d = snap.data(); 
  ...
  const updateProgress = async (fields) => {
    ...
    try { 
      await db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('progress').doc(activeEpisodeId).set(fields, { merge: true }); 
    } catch (err) {}
  };

  // --- FLATTEN DRILLS FOR PT UI ---
  ...
  const filteredVocab = useMemo(() => {
    ...
    return (globalDictionary || []).filter(w => !q || removeDiacritics(w.portuguese).includes(q) || removeDiacritics(w.english).includes(q));
  }, [searchTerm, globalDictionary]);

  const deleteWord = async (id) => {
    if (!user) return;
    const newDict = globalDictionary.filter(i => i.id !== id);
    await db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('database').doc('lexicon').set({ entries: newDict });
    setDeletingId(null);
  };
  ...
  const handleExportPrompt = async () => {
    ...
    try {
      const epsSnapshot = await db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('episodes')
          .orderBy('timestamp', 'desc').limit(5).get();
      
      let richContext = "";
      for (const docSnap of epsSnapshot.docs) {
        const ep = docSnap.data();
        const progSnap = await db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('progress').doc(docSnap.id).get();
        ...
        if (Array.isArray(ep.drills)) {
          ep.drills.forEach(d => { if (Array.isArray(d.examples)) d.examples.forEach(ex => { if (ex.portuguese) drillSentences.push(ex.portuguese); }); });
        }
        ...
        richContext += `\n--- Episode: ${ep.title} ---\nUser Request: ${ep.userPrompt || 'None'}\nReading: ${ep.reading?.portuguese}\nDrilled Target Words: ${ep.reading?.focus?.map(f=>f.word).join(', ')}\nDrill Sentences: ${drillSentences.join(' | ')}\nQuiz Sentences: ${quizQuestions.join(' | ')}\nQuiz Score: ${score}/${ep.quiz?.length || 15}\n`;
      }

      const vocabContext = globalDictionary.map(d => d.portuguese).join(', ');

      const systemInstruction = `You are an expert European Portuguese curriculum designer. Generate a highly structured B1/B2 lesson. 
...
{ "portuguese": "...", "english": "...", "pos": "Noun" }
      ...
      a.href = url;
      a.download = `Portuguese_Prompt_${Date.now()}.txt`;
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        batch.set(db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('episodes').doc(newEpisodeId), episodeDoc);
        batch.set(db.collection('artifacts').doc('portuguese-master').collection('users').doc(user.uid).collection('database').doc('lexicon'), { entries: combinedDictionary }, { merge: true });
        await batch.commit();
  ...
  // PLAY BUTTON COMPONENT (Enhanced with Pause feature)
  function PlayBtn({ id, textToSpeak, onMaster }) {
      ...
      return (
          <button 
              ...
              className={`flex items-center justify-center p-3 rounded-full transition-all border ${isPlaying ? 'bg-emerald-500/20 text-emerald-500 border-emerald-500/50 shadow-inner' : isDarkMode ? 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 border-zinc-700' : 'bg-white text-stone-500 hover:bg-stone-50 hover:text-emerald-600 border-stone-200 shadow-sm'}`}
          >
              {isPlaying ? <Pause size={18} className="animate-pulse text-emerald-500" /> : <Volume2 size={18} />}
          </button>
      );
  }
      ...
      <header className={`sticky top-0 z-50 w-full backdrop-blur-md transition-colors duration-300 border-b ${isDarkMode ? 'bg-zinc-950/85 border-zinc-800' : 'bg-white/90 border-stone-200'}`}>
        <nav className={`py-2 px-4 md:px-6 flex items-center justify-between border-b ${isDarkMode ? 'border-zinc-800/50' : 'border-stone-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="bg-emerald-600 text-white p-1.5 rounded-lg shadow-sm shadow-emerald-900/20"><Globe size={18} /></div>
            <span className="text-lg font-bold tracking-tight">Portuguese Cloud</span>
          </div>
          
          <div className="flex items-center gap-3 relative">
            <div className="relative">
              <button onClick={() => !isGenerating && setDropdownOpen(!dropdownOpen)} disabled={isGenerating} className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full border transition-all ${isDarkMode ? 'border-zinc-700 bg-zinc-800 text-emerald-400 hover:bg-zinc-700' : 'border-stone-200 bg-stone-50 text-emerald-700 hover:bg-stone-100'}`}>
              ...
              {dropdownOpen && (
                <div className={`absolute right-0 mt-2 w-64 rounded-2xl shadow-xl border overflow-hidden z-50 ${isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-stone-200'}`}>
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {episodesList.map(ep => (
                      <button key={ep.id} onClick={() => { setActiveEpisodeId(ep.id); setActiveTab('studio'); setDropdownOpen(false); }} className={`w-full text-left px-4 py-3 text-sm font-medium transition-colors ${activeEpisodeId === ep.id ? (isDarkMode ? 'bg-emerald-900/30 text-emerald-400' : 'bg-emerald-50 text-emerald-700') : (isDarkMode ? 'hover:bg-zinc-800 text-zinc-300' : 'hover:bg-stone-50 text-stone-700')}`}>
                        {ep.title}
                      </button>
                    ))}
                  </div>
            ...
            <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-1.5 rounded-full border transition-all active:scale-90 ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-emerald-400' : 'bg-stone-50 border-stone-200 text-emerald-600'}`}>
        ...
        <div className="max-w-4xl w-full mx-auto px-4">
          <div className="flex overflow-x-auto no-scrollbar">
            {[
              ...
              { id: 'reading', icon: BookOpen, label: 'Reading', disabled: !activeEpisode?.reading?.portuguese },
              ...
              <button key={tab.id} disabled={tab.disabled} onClick={() => handleTabSwitch(tab.id)} className={`py-3 px-3 sm:px-5 text-sm font-bold border-b-2 -mb-[1px] transition-all flex items-center gap-2 shrink-0 ${tab.disabled ? 'opacity-30 cursor-not-allowed' : activeTab === tab.id ? (isDarkMode ? 'border-emerald-500 text-emerald-400' : 'border-emerald-600 text-emerald-700') : 'border-transparent text-stone-500 hover:text-stone-700'}`}>
        ...
        {activeTab === 'studio' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            <header className="mb-4 flex items-center gap-4">
              <div className="p-4 bg-emerald-800 text-emerald-50 rounded-2xl shadow-lg"><MessageSquare size={32} /></div>
            ...
            <section className={cardClass}>
              ...
              <div className="flex flex-col gap-4">
                <input type="text" value={topicInput} onChange={e => setTopicInput(e.target.value)} disabled={isGenerating} placeholder="e.g., A mystery story set in Alfama, Lisbon..." className={`w-full px-4 py-3 rounded-xl border focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-100' : 'bg-stone-50 border-stone-200 text-stone-900'}`} />
              ...
              <div className="space-y-6 pt-4">
                ...
                {activeEpisode.tutorIntroduction && (
                  <div className="flex flex-col items-start animate-in fade-in">
                    <span className={`text-[10px] font-bold uppercase tracking-widest mb-1 mx-1 ${isDarkMode ? 'text-emerald-500' : 'text-emerald-600'}`}>Tutor</span>
                    <div className={`max-w-[85%] sm:max-w-[75%] p-4 sm:p-5 rounded-3xl rounded-tl-sm shadow-sm border ${isDarkMode ? 'bg-emerald-950/30 border-emerald-900/50 text-emerald-100' : 'bg-emerald-50 border-emerald-100 text-emerald-900'}`}>
                      <p className="text-lg leading-relaxed">{activeEpisode.tutorIntroduction}</p>
                      <div className="mt-4 flex flex-wrap gap-3">
                        <button onClick={() => setActiveTab('reading')} className={`text-sm font-bold px-4 py-2 rounded-xl transition-all ${isDarkMode ? 'bg-emerald-800/50 hover:bg-emerald-800' : 'bg-emerald-200/50 hover:bg-emerald-200 text-emerald-800'}`}>Go to Reading</button>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {activeTab === 'reading' && activeEpisode?.reading && (
          <div className="space-y-8 animate-in fade-in duration-300">
            <header className="mb-8 flex items-center gap-4">
              <div className="p-4 bg-emerald-800 text-emerald-50 rounded-2xl shadow-lg"><Globe size={32} /></div>
              <div><h2 className="text-2xl sm:text-3xl font-bold tracking-tight">{activeEpisode.title}</h2><p className={`text-sm mt-1 ${isDarkMode ? 'text-zinc-500' : 'text-stone-500'}`}>European Portuguese • B1/B2</p></div>
            </header>

            <section className={cardClass}>
              <div className={headerClass}>
                <div className="flex items-center gap-3"><BookOpen className="text-emerald-600" size={24} /><h2 className="text-xl sm:text-2xl font-bold tracking-wide">Reading</h2></div>
                <PlayBtn id="read-pt" textToSpeak={activeEpisode.reading.portuguese} />
              </div>
              <div className="prose prose-lg max-w-none leading-relaxed text-xl space-y-6">
                {typeof activeEpisode.reading.portuguese === 'string' && activeEpisode.reading.portuguese.split('\n\n').map((p, i) => <p key={i}>{p}</p>)}
              </div>
            </section>
            ...
            {activeEpisode.reading.english && (
              <section className={cardClass}>
                <div className={headerClass}>
                  <div className="flex items-center gap-3"><BookOpen className="text-emerald-600" size={24} /><h2 className="text-xl sm:text-2xl font-bold tracking-wide">Translation</h2></div>
            ...
            {Array.isArray(activeEpisode.reading.focus) && activeEpisode.reading.focus.length > 0 && (
              <section className={cardClass}>
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-stone-100 dark:border-zinc-800">
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold tracking-wide">Focus Vocabulário & Gramática</h2>
                </div>
                <div className="space-y-4">
                  {activeEpisode.reading.focus.map((item, idx) => (
                    <div key={idx} className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-zinc-950/45 border-zinc-800' : 'bg-stone-50 border-stone-200'}`}>
                      <h3 className="font-bold text-lg text-emerald-500 mb-2">{idx + 1}. {item.word}</h3>
            ...
            <header className="mb-8">
              <div className="flex justify-between items-end mb-4">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-emerald-800 text-emerald-50 rounded-2xl shadow-lg"><Layers size={32} /></div>
                  <div><h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Exercícios</h2></div>
                </div>
                <div className="text-right pb-1">
                  <span className="text-[10px] font-bold uppercase tracking-widest block mb-1 text-emerald-600">Progresso</span>
              ...
              <div className={`h-1.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-zinc-800' : 'bg-stone-200'}`}>
                <div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 transition-all duration-1000" style={{ width: `${drillsFlattened.length > 0 ? (Object.keys(progress.mastered || {}).length / drillsFlattened.length) * 100 : 0}%` }} />
              </div>
            </header>
                      ...
                      return (
                        <div key={idx} className={`rounded-2xl p-5 border shadow-sm transition-all duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-stone-200'}`}>
                          <div className={`flex items-center justify-between mb-4 border-b pb-3 ${isDarkMode ? 'border-zinc-800/85' : 'border-stone-200/50'}`}>
                            <div className="flex items-center gap-3">
                              <div className={`flex items-center justify-center w-6 h-6 rounded-full border ${isMastered ? 'bg-emerald-100 border-emerald-50 text-emerald-600' : isDarkMode ? 'border-zinc-700 text-zinc-800' : 'border-stone-300 text-stone-200'}`}><Check size={14} /></div>
                              <h4 className={`font-bold uppercase text-sm tracking-widest ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>EXEMPLO {idx + 1}</h4>
                            </div>
        ...
        {activeTab === 'quiz' && activeEpisode?.quiz && (
          <div className="space-y-8 animate-in fade-in duration-300">
            <header className="mb-8 flex justify-between items-end">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-emerald-800 text-emerald-50 rounded-2xl shadow-lg"><CheckCircle size={32} /></div>
                <div><h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Quiz</h2></div>
              </div>
              <div className="text-right pb-1">
                <span className={`text-[10px] font-bold uppercase tracking-widest block mb-1 ${isFullyGraded ? 'text-amber-500' : 'text-emerald-600'}`}>{isFullyGraded ? 'Pontuação' : 'Progresso'}</span>
                ...
                return (
                  <div key={idx} className={`${cardClass}`}>
                    <p className="text-xl font-medium mb-5 leading-relaxed">
                      ...
                      {typeof q.text === 'string' && q.text.split(/_{3,}/).map((part, i, arr) => (
                        <Fragment key={i}>
                            {part}
                            {i < arr.length - 1 && (
                            <span className={`mx-1 px-3 py-1 font-bold rounded-lg border text-base ${pAnswer ? (isGraded ? (pAnswer === q.correct ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-red-100 text-red-800 border-red-300 line-through') : (isDarkMode ? 'bg-emerald-500/20 text-emerald-300' : 'bg-emerald-100 text-emerald-900')) : (isDarkMode ? 'bg-zinc-800 text-zinc-500' : 'bg-slate-100 text-slate-400')}`}>{pAnswer || "___"}</span>
                            )}
                        </Fragment>
                        ))}
                    </p>
                    ...
                    {!progress.quizRevealed?.[qId] ? (
                      <button disabled={isGenerating} onClick={() => updateProgress({ quizRevealed: { ...progress.quizRevealed, [qId]: true } })} className={`flex items-center px-4 py-2.5 rounded-xl border text-xs font-bold uppercase ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-emerald-400' : 'text-emerald-700 bg-white border-emerald-200'}`}><Eye className="w-4 h-4 mr-2" /> Ver as opções</button>
                    ) : (
                      <div className="animate-in fade-in">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                          {Array.isArray(q.options) && q.options.map((opt, optIdx) => (
                            <button key={`${opt}-${optIdx}`} onClick={() => !isGraded && updateProgress({ quizAnswers: { ...progress.quizAnswers, [qId]: opt } })} disabled={isGraded || isGenerating} className={`px-4 py-3 rounded-xl border text-sm font-medium ${isGraded ? (q.correct === opt ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-600' : pAnswer === opt ? 'bg-red-500/10 text-red-400/50 line-through' : 'opacity-50') : (pAnswer === opt ? 'bg-emerald-600 text-white border-emerald-500' : isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-slate-300')}`}>{opt}</button>
                          ))}
                        </div>
                        <div className="flex justify-end mt-2">
                          {pAnswer && !isGraded && <button disabled={isGenerating} onClick={() => { updateProgress({ quizGraded: { ...progress.quizGraded, [qId]: true } }); handleSpeak(q.text.replace(/_{3,}/, q.correct)); }} className="flex items-center bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold"><CheckCircle className="w-4 h-4 mr-2" /> Verificar</button>}
        ...
        {activeTab === 'dictionary' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <header className="mb-8 flex flex-col md:flex-row justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-emerald-800 text-emerald-50 rounded-2xl"><Hash size={32} /></div>
                <div><h2 className="text-2xl font-bold">Cloud Dictionary</h2><p className="text-sm">{globalDictionary?.length || 0} palavras na sua base de dados.</p></div>
              </div>
              <div className="flex items-center gap-3 w-full md:w-auto">
                <div className="relative w-full md:w-64">
                  ...
                  <input 
                    ...
                    className={`w-full pl-11 pr-4 py-3 rounded-2xl border outline-none focus:ring-2 focus:ring-emerald-500/30 transition-all ${isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-stone-300'}`} 
                  />
                </div>
              </div>
            </header>
                ...
                <div key={item.id} className={`p-5 rounded-2xl border flex flex-col justify-between transition-all ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-stone-200'}`}>
                  <div>
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="text-xl font-serif font-medium">{item.portuguese}</h3>
                      <div className="flex gap-2 items-center">
                        {item.pos && <span className="text-[9px] uppercase font-bold tracking-wider px-2 py-1 rounded-md border text-emerald-600 dark:text-emerald-400">{item.pos}</span>}
                        {deletingId === item.id ? (
